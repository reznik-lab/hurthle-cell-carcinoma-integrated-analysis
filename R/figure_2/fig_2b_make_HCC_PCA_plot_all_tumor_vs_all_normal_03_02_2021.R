...
# add histology 2 for ggplot labeling
sampleInfo$HISTOLOGY3<-sampleInfo$HISTOLOGY
sampleInfo[sampleInfo$HISTOLOGY3 %in% "widely invasive HCC",]$HISTOLOGY3<-"HCC"
sampleInfo[sampleInfo$CLIENT_IDENTIFIER %in% clin_HWIDE_recurrent,]$HISTOLOGY3<-"HCC"
sampleInfo[sampleInfo$HISTOLOGY3 %in% "minimally invasive HCC",]$HISTOLOGY3<-"HCC"
sampleInfo[sampleInfo$HISTOLOGY3 %in% "HA",]$HISTOLOGY3<-"HA"
sampleInfo[sampleInfo$HISTOLOGY3 %in% "Poorly diff",]$HISTOLOGY3<-"PD"
sampleInfo[sampleInfo$HISTOLOGY3 %in% "PTCTV",]$HISTOLOGY3<-"PTCTV"
sampleInfo[sampleInfo$HISTOLOGY3 %in% "normal",]$HISTOLOGY3<-"Normal"
...
tumorSampleName<-tumorSampleName[!( tumorSampleName %in% removedSamples)]
...
selectedSamples<-c(tumorSampleName,normalSampleName)
#selectedSamples<-selectedSamples[!(selectedSamples %in% c("YD29","YD65")),]
...
samppca$group = sampleInfo[rownames(samppca),'HISTOLOGY3']
...
fileName<-"HCC_PCA_color_by_group_all_tumor_vs_all_normal_03_02_2021.pdf"
...
samppca$group<-factor(samppca$group,levels=c("Normal","HA","HCC","PD","PTCTV"))
#ggplot(samppca,aes(PC1,PC2,shape = Tissue)) + geom_point() + theme_minimal(base_size = 16)
...
p <- p + scale_color_manual(values=c("HA"="deepskyblue1",
                                     "HCC"="olivedrab3",
                                     "Normal"="grey55",
                                     "PD"="darkorchid1",
                                     "PTCTV"="darkorange2"))
#p <- p + geom_point(aes(shape=mutation))
p <- p + geom_hline(yintercept = 0, linetype="dashed",color="grey65")
p <- p + geom_vline(xintercept = 0, linetype="dashed",color="grey65")
...
fileName<-"HCC_PCA_color_by_group_all_tumor_vs_all_normal_legend_03_02_2021.pdf"
